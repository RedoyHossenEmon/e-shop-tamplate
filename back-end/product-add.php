...
function img_upload(){
  ...
  $savedFilename = "img/product/". pathinfo($originalFileName, PATHINFO_FILENAME) . $hexCode."." . $imageFileType;
...
$brand = ($_POST["pBrand"] == "None") ? "" : $row['pBrand'];;
$cetagory = ($_POST["pCetagory"] == "None") ? "" : $row['pCetagory'];;
$style = ($_POST["pStyle"] == "None") ? "" : $row['pStyle'];;
...
if (empty($product_name) || empty($price) ) {
  alertFunc("Please fill all nessesary info.");
    ...
    $sql = "INSERT INTO `productlist` ( `product_id`, `product_name`, `img_url`, `price`, `ex_price`, `brand`, `cetagory`, `style`, `description`, `product_tags`) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
